    ...
    public partial class frmCliente : Form
    {
        ...
        private void btnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    respuesta = NPersona.Actualizar(int.Parse(txtId.Text), "Cliente", "", txtNombre.Text, cmbTipDoc.SelectedItem.ToString(), txtNumDoc.Text, txtDirecc.Text, txtTel.Text, txtEmail.Text);
                    if (respuesta == "OK")
                    {
        ...
        private void btnEliminar_Click_1(object sender, EventArgs e)
        {
            try
            {
                DialogResult opcion = MessageBox.Show("Desea eliminar este Cliente?", "Sistema de ventas-Eliminar", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dgvListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            if (respuesta == "OK")
                            {
                                this.MensajeOK($"Se Elimino el Cliente: {row.Cells[3].Value}");
                            }
                            else
                            {
                                this.MensajeError(respuesta);
        ...
        private void btnInsertar_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    respuesta = NPersona.Insertar("Cliente", txtNombre.Text, cmbTipDoc.SelectedItem.ToString(), txtNumDoc.Text, txtDirecc.Text, txtTel.Text, txtEmail.Text);
                    if (respuesta == "OK")
                    {
        ...
        private void cmbTipDoc_ChangeUICues(object sender, UICuesEventArgs e)
        {

        }

        private void cmbTipDoc_Click_1(object sender, EventArgs e)
        {
            cmbTipDoc.Items.Clear();
